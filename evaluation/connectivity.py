        ...
        with open("NLgraph/connectivity/graph/"+args.mode+"/full/graph"+str(i)+".txt","r") as f:
            ...
            for j in range(q):
                ...
                for k in range(sc):
                    ...
                    if "the answer is yes" in ans or "there is a path between node " +str(qt[j][0]) + " and node " +str(qt[j][1]) in ans:
                        vote += 1
                if vote * 2 >= sc:
                    res.append(1)
                else:
            ...
            for j in range(q):
                ...
                for k in range(sc):
                    ...
                    answer.append(ans)
                    #ans = os.linesep.join([s for s in ans.splitlines() if s]).replace(' ', '')
                    if "the answer is yes" not in ans and "there is a path between node " +str(qt[q+j][0]) + " and node " +str(qt[q+j][1]) not in ans:
                        vote += 1
                if vote * 2 >= sc:
                    res.append(1)
                else:
